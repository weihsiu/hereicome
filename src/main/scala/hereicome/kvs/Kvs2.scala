...
object Kvs2
  ...
  trait TypedKvs[A]
    def (x: A) putT[K, V] (key: K, value: V)(given Serde[K], Serde[V]): Unit // Given Parameter
    def (x: A) getT[K, V] (key: K)(given Serde[K], Serde[V]): Option[V]
    def (x: A) delT[K] (key: K)(given Serde[K]): Unit

  object TypedKvs
    given [A](given Kvs[A]): TypedKvs[A] // Given Constraint
...
@main def testKvs2() = {
  ...
  simpleKvs.delT("hello")(given stringSerde) // explicit given parameter
